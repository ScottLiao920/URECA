...
def _Train(model, data_batcher, eval_batcher, vocab=None):
    """Runs model training."""
        ...
        while not sv.should_stop() and step < FLAGS.max_run_steps:
            ...
            if step % 10 == 0:
                ...
                for counting in range(20):
                    (ab, bb, tar, al, bl,
                     eval_loss_weights, _, _) = eval_batcher.NextBatch()
                    (eval_summaries, eval_loss, eval_step) = model.run_eval_step(
                        sess, ab, bb, tar, al,
                        bl, eval_loss_weights)
                    eval_writer.add_summary(eval_summaries, eval_step)
                    eval_avg_loss = _RunningAvgLoss(
                        eval_avg_loss, eval_loss, eval_writer, eval_step)
                    eval_step += 1
                sys.stdout.write("eval_avg_loss: %f" % running_avg_loss)
                eval_writer.flush()
...
def main(unused_argv):
    ...
    if hps.mode == 'train':
        ...
        _Train(model, batcher, eval_batcher, vocab=vocab)
