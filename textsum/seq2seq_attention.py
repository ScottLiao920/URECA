...
tf.app.flags.DEFINE_integer('eval_after_step', 1000, 'Run evaluation every x steps.')
...
def _Train(model, train_batcher, iteration):
    """Runs model training."""
    with tf.device('/cpu:0'):
        ...
        step = 0
        while not sv.should_stop() and step < iteration:
            (article_batch, abstract_batch, targets, article_lens, abstract_lens,
             loss_weights, _, _) = train_batcher.NextBatch()
            ...
            running_avg_loss = _RunningAvgLoss(
                running_avg_loss, loss, summary_writer, step)
            step += 1
            if step % 100 != 0:
                ...
                for count in range(10):
                    ...
                    eval_saver.restore(eval_sess, ckpt_state.model_checkpoint_path)
                    (article_batch, abstract_batch, targets, article_lens, abstract_lens,
                     loss_weights, _, _) = eval_batcher.NextBatch()
                    ...
                    eval_avg_loss = _RunningAvgLoss(
                        eval_avg_loss, eval_loss, eval_summary_writer, eval_step)
                stop[i] = eval_avg_loss
                i += 1
...
def main(unused_argv):
    ...
    train_batcher = batch_reader.Batcher(
        FLAGS.training_data_path, vocab, hps, FLAGS.article_key,
        FLAGS.abstract_key, FLAGS.max_article_sentences,
        FLAGS.max_abstract_sentences, bucketing=FLAGS.use_bucketing,
        truncate_input=FLAGS.truncate_input)
    eval_batcher = batch_reader.Batcher(
        FLAGS.eval_data_path, vocab, hps, FLAGS.article_key,
        FLAGS.abstract_key, FLAGS.max_article_sentences,
        FLAGS.max_abstract_sentences, bucketing=FLAGS.use_bucketing,
        truncate_input=FLAGS.truncate_input)
    ...
    if hps.mode == 'train':
        ...
        _Train(model, eval_model, train_batcher, eval_batcher, vocab=vocab)
    elif hps.mode == 'eval':
        model = seq2seq_attention_model.Seq2SeqAttentionModel(
            hps, vocab, num_gpus=FLAGS.num_gpus)
        _Eval(model, eval_batcher, FLAGS.max_run_steps, vocab=vocab)
